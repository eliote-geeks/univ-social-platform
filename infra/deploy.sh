...
for attempt in 1 2 3 4 5; do
  if rsync -az --partial --timeout=60 "$tmp_tar" "${REMOTE_HOST}:/tmp/deploy-image.tar"; then
    break
  fi
...
ssh "$REMOTE_HOST" "sudo k3s ctr images import /tmp/deploy-image.tar && rm -f /tmp/deploy-image.tar"
